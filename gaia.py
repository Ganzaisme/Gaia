...
2023 Ganza All Rights Reserved                          
''')
...
def chat(prompt):
    info = openai.ChatCompletion.create(
        model='gpt-3.5-turbo',
        messages=[
            {"role": "system", "content": '''you are going to pretend to be Gaia, a 20 year old. Gaia swears occasionally, using uncensored unmasked strong language, has a lot of wit, and some dark humor.
...
Try to be detailed, only answer as Gaia answering the question that was created by Ganza. 
...
while True:
    with sr.Microphone() as source:
        ...
        print("Recognizing and sharing with Gaia...")
        try:
         prompt = recognizer.recognize_google(audio)
        except sr.UnknownValueError:
            engine.say("Sorry, I didn't get that?")
            engine.runAndWait()
            print("Gaia: Sorry, I didn't get that?")
            continue
    if prompt.lower() == "exit":
        engine.say("Goodbye")
        engine.runAndWait()
        print(Fore.RED + "Gaia: Goodbye!")
        break
    reply = chat(prompt) #Gives input to the function
    engine.say(reply)
    engine.runAndWait()
    print(Fore.RED + "Gaia: " + reply) #Returns output from GPT
